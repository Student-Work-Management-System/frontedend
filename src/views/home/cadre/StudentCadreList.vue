...
const deleteStudentCadreLogic = async () => {
  ...
  let reqs = studentIds.map(id => (async () => {
...
<template>
  <v-card elevation="10" height="100%" width="100%">
    <EditStudentCadreForm v-model="editStudentCadreFormDialog" :info="editInfo" @on-closed="() => {
      ...
      <v-card prepend-icon="mdi-delete" title="删除选择" :text="`已选择 ${selected.length} 条记录，本操作不可撤回，确定要删除吗？`">
        <v-card-actions class="mx-auto">
          <v-btn :loading="loading" :disabled="selected.length === 0" color="error"
            @click="deleteStudentCadreLogic">删除</v-btn>
    ...
    <section class="menu">
      ...
      <span class="w-10">
        <v-select label="职位等级" v-model="query.cadreLevel" :items="CadreLevels" class="text-indigo" color="indigo"
          hide-details clearable variant="underlined">
        </v-select>
      </span>
      <span class="w-10 text-indigo">
        <SemesterSelect v-model="query.appointmentStartTerm" color="'indigo'" :label="'任职开始学期'" variant="underlined" />
      </span>
      <span class="w-10 text-indigo">
        <SemesterSelect v-model="query.appointmentEndTerm" :color="'indigo'" :label="'任职结束学期'" variant="underlined" />
      </span>

      <span class="w-15 text-indigo">
        <v-text-field v-model="query.search" color="indigo" @update:modelValue="fetchStudentCadreLogic" :loading="loading"
      ...
      <v-btn prepend-icon="mdi-refresh" v-if="has('student:select') &&
        ...
        has('cadre:select') &&
        has('major:select')
        " @click="fetchStudentCadreLogic">刷新</v-btn>

      <v-btn v-if="has('student_cadre:delete')" prepend-icon="mdi-delete" color="error"
    ...
    <section class="pa-4 w-100" ref="tableDom">
      <v-card>
        <v-data-table-server v-model="selected" :headers="headers" :height="tableHeight" :items="data"
          :items-length="dataLength" :loading="loading" v-model:page="query.pageNo"
          v-model:items-per-page="query.pageSize" @update:options="loadItems" show-select return-object>
          <template v-slot:item.operations="{ item }">
            <div>
              <v-btn v-if="has('student_cadre:update')" prepend-icon="mdi-pencil" color="indigo" @click="() => {

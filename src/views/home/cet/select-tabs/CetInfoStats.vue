...
const updateChart = () => {
  loading.value = true
  if (cet4ParticipateStateChart === undefined || cet6ParticipateStateChart === undefined || scoreChart === undefined) {
    cet4ParticipateStateChart = new VChart(parsePieISpec("CET4通过率", stats.value.cet4.state), { dom: cet4ParticipateChartRef.value })
    cet4ParticipateStateChart.renderSync()

    cet6ParticipateStateChart = new VChart(parsePieISpec("CET6通过率", stats.value.cet6.state), { dom: cet6ParticipateChartRef.value })
    cet6ParticipateStateChart.renderSync()

    scoreChart = new VChart(parseBarISpec("考试分数", stats.value?.score), { dom: scoreChartRef.value })

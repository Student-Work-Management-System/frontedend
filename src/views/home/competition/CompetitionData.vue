...
const fetchStudentCompetitionLogic = async () => {
  ...
  data.value = result.data.records.flatMap((item) =>
...
const fixHeight = () => {
  ...
  tableHeight.value = (totalHeight - offsetTop) * 0.76 - padding
}
onMounted(() => {
  fixHeight()
...
<template>
  <v-card elevation="10" height="100%" width="100%">
    ...
    <div class="pt-6 px-4">
      <v-alert
        text="如果删除任何一条团队类型的竞赛记录，相同团队中学生对应记录(相同竞赛上报ID)也会被删除！"
        title="提示："
        type="warning"
        variant="outlined"
      ></v-alert>
    ...
    <section class="menu">
      ...
      >
      <v-btn
        v-if="has('student_competition_claim:delete')"
        ...
        >
          ...
          <template v-slot:item.members="{ item }">
            <v-chip
              ...
              v-for="(member, id) in item.members"
              :key="id"
            >
              {{ member }}
            </v-chip>

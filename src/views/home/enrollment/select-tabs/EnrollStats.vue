...
const getStatsDataHandler = async () => {
  ...
  stats.value.enrollmentState = [
    {
      ...
      value: toFixDot2(
        (stats.value.enrollmentState['第一志愿录取人数'] * 100) /
          stats.value.enrollmentState['总录取人数']
      )
    },
    {
      type: '第一志愿未录取',
      number:
        stats.value.enrollmentState['总录取人数'] - stats.value.enrollmentState['第一志愿录取人数'],
      value:
        100 -
        toFixDot2(
          (stats.value.enrollmentState['第一志愿录取人数'] * 100) /
  ...
  stats.value.origin = Object.keys(stats.value.origin).map((key) => ({
    type: key,
    value: stats.value?.origin[key]
  }))
  const originTotal = stats.value.origin?.reduce(
    (total: number, item: { key: string; value: number }) => total + item.value,
    0
  )
  stats.value.origin = stats.value.origin.map((item: { key: string; value: number }) => ({
    ...item,
    value: toFixDot2((item.value * 100) / originTotal),
    number: item.value
  }))
  ...
  stats.value.regionScores = Object.keys(stats.value.regionScores)
    ...
    .flatMap((pItem) =>
      Object.keys(pItem.value).map((key) => ({
        type: pItem.type,
        level: key,
        value: pItem.value[key]
      }))
    )
...
const updateChart = () => {
  ...
  if (enrollStateChart === undefined) {
    enrollStateChart = new VChart(parsePieISpec('录取人数', stats.value?.enrollmentState), {
      dom: enrollStateChartRef.value
    })
    enrollStateChart.renderSync()

    originChart = new VChart(parsePieISpec('生源地', stats.value?.origin), {
      dom: originChartRef.value
    })
    originChart.renderSync()

    scoreChart = new VChart(parseBarISpec('工作地点', stats.value?.regionScores), {
...
<template>
  <section class="h-100 d-flex flex-column">
    <section class="menu">
      ...
      >
      <v-btn
        v-if="has('enrollment:select') && has('file:download')"

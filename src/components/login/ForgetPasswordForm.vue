        ...
        <v-window-item :value="1">
          <v-container class="px-8">
            <v-text-field
              ...
              clearable
              required
              :rules="[() => !!updateModel.username || '该选项必填！']"
            >
              <v-tooltip activator="parent" location="top"
                >账户名一般为 教师工号 或 学生学号</v-tooltip
              >
            </v-text-field>
          </v-container>
          ...
          <v-container class="w-100 d-flex justify-space-evenly">
            <v-btn
              :disabled="!(updateModel.username.length > 0)"
              text="下一步"
              :loading="loading"
              color="indigo"
              variant="plain"
              @click="findBackPasswordHandler"
            ></v-btn>
        ...
        <v-window-item :value="2">
          <v-container class="px-8">
            <v-form v-model="updateForm">
              <v-alert
                class="mb-6"
                text="激活邮件已发送到以下邮箱，请在 5 min 内激活！"
                title="成功"
                type="success"
              ></v-alert>
              <v-text-field
              ...
              <v-text-field
                ...
                required
                :rules="[() => !!updateModel.password || '该选项必填！']"
              ></v-text-field>
              <v-text-field
                label="确认密码"
                prepend-icon="mdi-lock"
                v-model="updateModel.repassword"
                required
                :rules="[

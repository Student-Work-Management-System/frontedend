        ...
        <v-card :loading="loading" width="auto" prepend-icon="mdi-account-plus" title="新增用户">
          <v-container class="px-8">
            <v-form v-model="form" class="form">
              <v-text-field label="用户名" v-model="newUser.username" prepend-inner-icon="mdi-account" :counter="20"
                clearable required :rules="[() => !!newUser.username || '该选项必填！']">
                <template v-slot:prepend>
                  <v-icon size="smaller" color="error" icon="mdi-asterisk"></v-icon>
                </template>
              </v-text-field>
              <v-text-field label="密码" prepend-inner-icon="mdi-lock" v-model="newUser.password" :counter="18" clearable
                required :rules="[() => !!newUser.password || '该选项必填！']">
                <template v-slot:prepend>
                  <v-icon size="smaller" color="error" icon="mdi-asterisk"></v-icon>
                </template>
              </v-text-field>

              <v-text-field label="真实姓名" v-model="newUser.realName" prepend-inner-icon="mdi-human-greeting" :counter="20"
                clearable required :rules="[() => !!newUser.realName || '该选项必填！']">
                <template v-slot:prepend>
                  <v-icon size="smaller" color="error" icon="mdi-asterisk"></v-icon>
                </template>
              </v-text-field>
              <v-text-field prepend-inner-icon="mdi-email" label="邮箱" v-model="newUser.email" :counter="11" clearable
                required :rules="[() => !!newUser.email || '该选项必填！']">
                <template v-slot:prepend>
                  <v-icon size="smaller" color="error" icon="mdi-asterisk"></v-icon>
                </template>
              </v-text-field>
        ...
        <v-card :loading="loading" prepend-icon="mdi-script" title="选择角色">
          <v-container class="w-100">
            ...
            <v-card-actions>
              ...
              <v-btn :disabled="!selected.length" :loading="loading" color="indigo" variant="text" @click="AddUserLogic">
                提交
              </v-btn>

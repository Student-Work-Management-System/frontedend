    ...
    <v-card :loading="loading" width="auto" prepend-icon="mdi-pencil" title="修改信息">
      <v-container class="px-8">
        <v-form v-model="form" class="form text-indigo">
          <v-text-field label="真实姓名" color="indigo" v-model="userInfo.realName" prepend-inner-icon="mdi-human-greeting"
            :counter="20" clearable required :rules="[() => !!userInfo.realName || '该选项必填！']">
            <template v-slot:prepend>
              <v-icon size="smaller" color="error" icon="mdi-asterisk"></v-icon>
            </template>
          </v-text-field>
          <v-text-field prepend-inner-icon="mdi-email" label="邮箱" color="indigo" v-model="userInfo.email" :counter="11"
            clearable required :rules="[() => !!userInfo.email || '该选项必填！']">
            <template v-slot:prepend>
              <v-icon size="smaller" color="error" icon="mdi-asterisk"></v-icon>
            </template>
          </v-text-field>
          <v-text-field label="密码" prepend-inner-icon="mdi-lock" color="indigo" v-model="userInfo.password" class="ml-7"
            :counter="18" clearable>
          </v-text-field>
        </v-form>
      </v-container>
      <v-divider></v-divider>

...
import { apiUpdateScholarships, scholarshipLevels } from '@/api/scholarship'
...
const info = defineModel<Scholarship>('info')
const localInfo = ref<Scholarship>()
watchEffect(() => {
...
const updateInfoLogic = async () => {
  loading.value = true
  const { data: result } = await apiUpdateScholarships(localInfo.value as Scholarship)
  if (result.code !== 200) {
...
<template>
  <v-dialog width="auto" min-width="500" height="auto" v-model="model">
    <v-card width="auto" prepend-icon="mdi-account-edit" title="奖学金信息">
      <v-container>
        <v-form v-model="form" class="px-8">
          <v-text-field
            ...
            color="indigo"
            label="奖学金名称"
            v-model="localInfo!.scholarshipName"
            required
          >
          ...
          <ItemSelect
            v-model="localInfo!.scholarshipLevel"
            label="奖学金级别"
